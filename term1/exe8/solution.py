...
def move_ball(ball, mvm):
    return (ball[0] + mvm[0], ball[1] + mvm[1])
  
class BallProblem(Problem):
    def __init__(self, initial,matrix, blocks):
        super().__init__(initial)
        self.matrix = matrix
        self.blocks = blocks        
        self.goal = (matrix // 2, matrix - 1)
    ...
    def successor(self, state):
        ...
        for i in range(len(balls)):
            for mvm, mvm_val, btw in zip(ball_movement, ball_movement_val, ball_between):
                prev_balls = list(balls)
                new_ball = move_ball(prev_balls[i], mvm_val)
                if self.valid_position(new_ball):
                    coord_in_between = move_ball(prev_balls[i], btw)
                    if coord_in_between in prev_balls:
                        old_x, old_y = prev_balls[i]
                        prev_balls[i] = new_ball
                        prev_balls = [tuple(b) for b in prev_balls if b != coord_in_between]
                        successors[f"{mvm} (x={old_x},y={old_y})"] = tuple(prev_balls)

        return successors
    
    def valid_position(self, coord):
        x, y = coord
        if coord in self.blocks:
            return False
        return 0 <= x < self.matrix and 0 <= y < self.matrix
        
def split_input_int(str, delimiter):
    return tuple([int(el) for el in str.split(delimiter)])
...
if __name__ == "__main__":
    ...
    for i in range(0, num_balls):
        temp = split_input_int(input(), ",")
        balls.append(temp)  
    num_blocks = int(input())
    for i in range(0, num_blocks):
        temp = split_input_int(input(), ",")
        blocks.append(temp)  
    balls = tuple(tuple(b) for b in balls)
    blocks = tuple(tuple(b) for b in blocks)
    problem = BallProblem(balls, size, blocks)
