...
def split_set(set, size):
    class_0 = [row for row in set if row[-1] == 0]
    class_1 = [row for row in set if row[-1] == 1]
    size_0 = int(size * len(class_0))
    size_1 = int(size * len(class_1))
    train = class_0[:size_0] + class_1[:size_1]
    test = class_0[size_0:] + class_1[size_1:]
    return train, test
def split_features(set): 
...
if __name__ == '__main__':
    size = int(input()) / 100
    train_set, test_set = split_set(dataset, size)
    
    train_X, train_Y = split_features(train_set)
    test_X, test_Y = split_features(test_set)
    
    accs = []
    for model in models:
        model.fit(train_X, train_Y)
        testPrediction = model.predict(test_X)
        acc = accuracy_score(test_Y, testPrediction)
        accs.append(acc)
    
    max_acc = max(accs)
    max_idx = accs.index(max_acc)
    print(f'Najgolema tocnost ima klasifikatorot {names[max_idx]}')

    combined_Y = []
    for x in test_X:
        combined_predictions = [model.predict([x])[0] for model in models]
        predictions = {
            0: 0,
            1: 0
        }
        
        #Tezina 2 najdobar klasifikator
        combined_predictions = combined_predictions + combined_predictions[max_idx]
        ...
        # if binary can be like this -> 
        # total = sum(combined_predictions) + combined_predictions[max_idx]
        # combined_Y.append(1 if total > 2 else 0)
        # So klasno sortiranje
        combined_Y.append(max_key)
    rec_score = recall_score(test_Y, combined_Y)
    print(f'Odzivot na kolekcijata so klasifikatori e {rec_score}')
