...
def split_set_features(set):
    return [row[:-1] for row in set], [row[-1] for row in set]

def split_set(set, percent):
    part = int(percent / 100 * len(set))
    return set[:part], set[part:]

if __name__ == '__main__':
    percent = 85
    dataset = [[float(i) for i in row] for row in dataset]
    train_set, test_set = split_set(dataset, percent)
    train_X, train_Y = split_set_features(train_set)
    test_X, test_Y = split_set_features(test_set)
