...
def adapt(row):
    rate = row[-1]
    row[-1] = 1 if rate >= 5 else 0
    return row
def split_features(set):
    return [row[:-1] for row in set], [row[-1] for row in set]

def split_set(size, set):
    perc = int(size / 100 * len(set))
    return dataset[:perc], dataset[perc:]

def remove_col(set, id):
    return [[value for index, value in enumerate(row) if index != id] for row in set]
...
if __name__ == '__main__':
    ...
    dataset = [adapt(row) for row in dataset]
    
    train_set, test_set = split_set(x, dataset)
    ...
    imp = list(DTc.feature_importances_)
    idx = imp.index(min(imp))
    train_X = remove_col(train_X, idx)
    test_X = remove_col(test_X, idx)
    classifierMLP = MLPClassifier(hidden_layer_sizes=15,
                                activation='relu',
    ...
    ssTrainX = ss.transform(train_X)
    ssTestX = ss.transform(test_X)
    mmTrainX = mm.transform(train_X)
    mmTestX = mm.transform(test_X)
    
    classifierMLP.fit(ssTrainX, train_Y)
    acc1 = classifierMLP.score(ssTestX, test_Y)
    
    classifierMLP.fit(mmTrainX, train_Y)
    acc2 = classifierMLP.score(mmTestX, test_Y)
    
    print(f"Accuracy with StandardScaler: {acc1}")
    print(f"Accuracy with MinMaxScaler: {acc2}")