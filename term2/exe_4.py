...
if __name__ == "__main__":
    ...
    train_set, test_set = split_set(dataset, 85)
    train_X, train_Y = split_features(train_set)
    test_X, test_Y = split_features(test_set)

    train_X = remove_column(train_X, col_index)
    test_X = remove_column(test_X, col_index)


    RFC = RandomForestClassifier(n_estimators=num, criterion=criteria, random_state=0)
    ...
    new_sample = [float(el) for el in new_sample]
    del new_sample[col_index]

    predict = RFC.predict([new_sample])[0]
    predict_val = RFC.predict_proba([new_sample])[0]
    print(predict)
    print(predict_val)
